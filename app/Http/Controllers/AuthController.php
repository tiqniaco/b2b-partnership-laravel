        ...
        try {
            ...
            if (Auth::attempt($credentials)) {
                ...
                if ($user->role == 'provider') {
                    ...
                    if ($user->role == 'provider' && $provider->verified_code == '0') {
                        return response()->json([
                            'status' => 'error',
    ...
    public function register(Request $request)
    {
        try {
            $request->validate([
                ...
                'government_id' => 'nullable|string|exists:governments,id',
...
//            );
    ...
    public function getVerifyCode(Request $request)
    {
        try {
            ...
            return response()->json([
                'status' => "success",
                'verified_code' => $provider->verified_code,
            ], 200);
        } catch (\Illuminate\Database\Eloquent\ModelNotFoundException $e) {
            return response()->json([
                'status' => 'error',
    ...
    public function switchClientAccount(Request $request)
    {
        try {
            $request->validate([
            ...
            if ($client->provider_id) {
                $provider = DB::table("provider_details")
                    ->where("provider_id", $client->provider_id)
                    ->first();
